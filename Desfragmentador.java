...
public final class Desfragmentador extends JFrame implements ActionListener {
    ...
    class Cola_de_Digitos implements Serializable{
        ...
        public Cola_de_Digitos(Color color, int digito, int j) {
            ...
            for (int i = 0; i < n_digitos; i++) {
            	...
            	//Le agrega el digito a la capa de numeros
                Tablero[0][i][j] = digito++;
                Casilla[i][j].setText(String.valueOf(Tablero[0][i][j]));
                //Agregamos el color a la capa de numeros
                Tablero[1][i][j] = color;
    ...
    //Un candado representa una combinacion de numeros o sea, una fila
    ...
    **************************************************************************/    
    public Desfragmentador(int n, int m){
        this.n = n;
        this.m  = m;
    ...
    **************************************************************************/     
    public void iniciaComponentes(){
        ...
        //Casilla es un objeto del tipo JButton
        //El objeto casilla representa la cuadricula
        Casilla = new JButton[n][m];
        //Declaramos el panel que contendra el tablero
        //Panel_Matriz es un objeto del tipo JPanel
        ...
        iniciaDigitos(n, m);
        //Finalmente agregamos el panel al  que creamos al incio de la aplicación
    ...
    //Este metodo es el que agrega las casillas al panel
    public void iniciaDigitos(int n, int m){
        ...
        //Se inicializa el tablero con algunos valores
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < m; j++){
                Tablero[0][i][j] = 0;
                Tablero[1][i][j] = Color.BLACK;
            }
        }
             
        //Empieza la declaración del tablero mediante un for doble
        //Se agregan las casillas al panel 
        //Se inicializan los valores de las casillas
        for (int i = 0; i < n; i++) {
            for (int  j = 0; j < m; j++) {
                ...
                Casilla[i][j].setBackground(Color.GREEN);//No pasa nada
                //Establecer un color para el texto del Casilla
                Casilla[i][j].setForeground(Color.GREEN);//No pasa nada
    ...
    **************************************************************************/    
    ...
    private void comienzaSimulacion(){
        ...
        Random r = new Random();
        //Mostrar solucion
        final LinkedList<Object[][][]> resultado = new LinkedList<>();
        do{    
        ...
        final int tamaño = resultado.size();
    ...
    public static void main(String[] args) {
        ...
        java.awt.EventQueue.invokeLater(()-> {
        	//Candado de 10 filas por 5 columnas
            new Desfragmentador(20, 15).setVisible(true);
        });
    }    
}
